...
import { CELL, COLS, rowIndex, ROWS } from './sheet.ts'
import { STYLES, type Config, type PetSettings } from './config.ts'
...
export function mountPet(
  ...
  const brainConfig = brain as unknown as { config: AutonomyConfig }
  let dragged = false
  applyGeometry()
  ...
  let recentToolTimer: number | undefined

  ctx.effect(() => ctx.on('tools/execute', (_exec, next) => {
    toolsInFlight++
    recentTool = true
    if (recentToolTimer !== undefined) window.clearTimeout(recentToolTimer)
  ...
  ctx.effect(() => ctx.on('approval/request', (_request, next) => {
    ...
    const settle = (): void => { waiting = false; sample() }
  ...
  let dragState: { x: number; y: number; left: number; top: number; moved: boolean } | undefined

  /**
   * Read the pet's current position as explicit pixel offsets.
   * @returns the left and top offsets.
   */
  const currentPosition = (): { left: number; top: number } => {
