...
const { Button, Switch } = __require("@deepseek-ai/dsh-client-ui-primitives");
/**
...
.cdpc-corners { display: grid; grid-template-columns: repeat(2, 20px); gap: 4px; }
.cdpc-corner {
...
.cdpc-reset {
  appearance: none; border: 0; background: none; padding: 0 2px;
...
function Chevron() {
    return h('svg', { className: 'cdpc-chevron', viewBox: '0 0 14 14', 'aria-hidden': 'true' }, h('path', {
        d: CHEVRON_PATH,
        fill: 'none',
        stroke: 'currentColor',
        strokeWidth: 1.6,
...
function ChickenPetCard(props) {
    ...
    }, h('span', { className: 'cdpc-headtext' }, h('span', { className: 'cdpc-name' }, '🐔 小鸡桌宠'), h('span', { className: 'cdpc-desc' }, '一只会自己找事做的像素鸡，跟着 Agent 的工作状态活动。')), dirty ? h('span', { className: 'cdpc-pending' }, '未保存') : null, h(Chevron)), open
